...
db_endpoint = open("/home/ec2-user/phonebook1/dbserver.endpoint", 'r', encoding='UTF-8') 
                                        ## enponiti github gönderecek oradan cekecegiz. Burada repo ismi degistirilebilir.
...
app.config['MYSQL_DATABASE_HOST'] = db_endpoint.readline().strip()      ## Buraya RDS instans direk konuluyor ama suan yok.
app.config['MYSQL_DATABASE_USER'] = 'admin'
app.config['MYSQL_DATABASE_PASSWORD'] = 'Oliver_1'          ## Bu bilgiler RDS olusturuken ayni olmasi lazim.
...
cursor = connection.cursor()                ## SOnrasi python kodlari.
...
if __name__== '__main__':
    init_phonebook_db()
    # app.run(debug=True)
    app.run(host='0.0.0.0', port=80)  ### En son da hangi port dan dagitiyorsa onu acacagiz.
